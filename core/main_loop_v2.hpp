...
inline std::optional<std::string> Node::OnWake(bool& leader_contact) {
    ...
    auto el_handler = [&leader_contact, this](NodeMessage&& message) -> std::optional<std::string> {
        ...
        return err ? err : std::optional<std::string>{};
    };

    for (size_t i = 0; i < EVENT_LOOP_THREADS; ++i) {
        NodeMessage nmsg;
    ...
    auto client_handler = [this](ClientMessage&& message) -> std::optional<std::string> {
...
inline std::optional<std::string> Node::MainLoop() {
    ...
    while (running_) {
        ...
        #ifdef DEBUG
        std::cout << "Main thread - found " << n << " ready fds\n";
        #endif
